...
for (ss_k, ss_v), (mf_k, mf_v) in zip(
    snapshot_stats.data_metafeatures.items(), mlflow_stats.items()
):
...
# %%
rep_model = model
mlflow_model = mlflow.pytorch.load_model(f"{run.info.artifact_uri}/model").to(
    rep_model.device
)
# %%
"""

...
class TestCallback:
    ...
    def test_transform_logging_callback(self):
        ...
        with modlee.start_run() as run:
            ...
            try:
                trainer.fit(model=self.model, train_dataloaders=img_dataloader)
            except:
                pass
        assert "transforms.txt" in os.listdir(
            os.path.join(modlee.last_run_path(), "artifacts")
        )

    @pytest.mark.training
    def test_checkpoint_callback(self):
        with modlee.start_run() as cur_run:
            trainer = modlee.Trainer(max_epochs=3)
            trainer.fit(model=self.model, train_dataloaders=self.dataloader)
    ...
    def test_data_metafeatures_callback(
        self,
    ):
